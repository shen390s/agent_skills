...
Rules for commands without a full-screen UI: naming, help, flags, output, errors,
prompts, config, and longevity. Load this for any CLI or one-shot command.
...
- **Suggest corrections** when input looks like a typo: `Did you mean ps? [y/n]` —
  ...
  Add a one-line description of what the suggestion does, not just its name.
...
- **Full-length versions for every flag** (`-h` and `--help`); reserve single-letter
  flags for common flags to avoid polluting the short namespace. Never use uppercase
  short flags unless the lowercase is already taken by a closely related concept.
- **Document every flag.** An undocumented flag becomes de-facto API the moment users
  find it — either make it official (docs, tests, stability) or remove it.
...
- **Support `--` to end flag parsing.** Everything after `--` is a positional argument,
  even if it starts with `-` — critical when passing flags through to a wrapped tool.
- **Cluster short boolean flags** (`-xvf` instead of `-x -v -f`).
...
- **Human-readable output is paramount.** Detect TTY and adapt (no animations/colors
  when stdout isn't a TTY). Layer machine surfaces (`--json`, `--quiet`) on top of human
  defaults — never strip tables, help, or confirmation to be "agent-ready".
- **Provide `--json`** (and `--csv`) for structured output; pairs with `jq`. A real
  machine contract needs more than the flag: field stability tied to semver, one
  consistent envelope across commands, `--fields`/`--schema` selection, and structured
  JSON errors on stderr too.
...
- **When truncating, teach how to narrow.** Say how many results were shown of how many
  total, and print the command that narrows them ("Showing 25 of 312 — add
  `--status published --since 7d`").
...
- **Use a pager** for lots of text (`less -FIRX`), only when stdin/stdout is a TTY.
  A pager is a real obstacle for new users (they may not know `q` exits) — don't default
  to one, or make it clearly indicated and disableable (`--no-pager`).
...
- **Emit machine-readable error fields.** In `--json` mode, include a stable `code`, a
  `suggestion` (prose), and a verbatim `fix_command` (plus `retryable`/`retry_after_ms`
  for transient failures) so agents and scripts can act without re-deriving the fix.
...
- **Unexpected errors:** provide debug/traceback info and bug-report instructions, but
  ...
  (pre-filled URL).
- **Exit codes:** 0 on success, non-zero on failure, mapped to major failure modes. Use
  the BSD `sysexits.h` codes where they apply — e.g. 64 usage error, 74 I/O error — so
  scripts can branch on the failure mode. Distinguish retryable vs not: rate-limited,
  timeout, unavailable → retry after a delay; usage, permission → don't retry.

## Prompting & interactivity

- **Only prompt if stdin is a TTY.** If not, skip prompting and require the flag/arg
  (error telling the user which flag to pass). Consider auto-detecting CI/agent
  environments and enabling non-interactive mode, but keep a documented opt-out env var.
